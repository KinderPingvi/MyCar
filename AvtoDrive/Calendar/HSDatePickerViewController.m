...
- (NSDateFormatter *)dateFormatter {
    if (!_dateFormatter) {
        ...
        _dateFormatter.locale =[[NSLocale alloc]initWithLocaleIdentifier:@"ru_RU"];
...
- (NSDateFormatter *)monthAndYearLabelDateFormater {
    if (!_monthAndYearLabelDateFormater) {
        ...
        _monthAndYearLabelDateFormater.locale =[[NSLocale alloc]initWithLocaleIdentifier:@"ru_RU"];
    }
...
- (NSString *)confirmButtonTitle {
    if (!_confirmButtonTitle) {
        _confirmButtonTitle = NSLocalizedString(@"Выбрать", @"HSDatePicker confirm button title");
    }
...
- (NSString *)backButtonTitle {
    if (!_backButtonTitle) {
        _backButtonTitle = NSLocalizedString(@"Назад", @"HSDatePicker back button");
    }
...
- (NSString *)stringDateForRow:(NSUInteger)row {
    ...
    if ([self isDate:date sameDayAsDate:[NSDate date]]) {
        return NSLocalizedString(@"Сегодня", @"Current day indicator");
    }
